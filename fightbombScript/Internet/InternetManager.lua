...
function InternetManager:sendCallScore(index)
    local strMsg = string.format("BYTE, 1,%d,", index)
    FGameDC:getDC():SendGameMsg(SUB_C_CALL_SCORE, strMsg)
end

function InternetManager:sendSelectMain(color)
    local strMsg = string.format("BYTE, 1,%d,", color)
    FGameDC:getDC():SendGameMsg(SUB_C_SELECT_CARD, strMsg)   
end
...
--发送吼牌
function InternetManager:sendBaoCard(bBaoCard)
    -- body
    local strMsg = "bool, 1, " .. (bBaoCard and 1 or 0) .. ","
    
    FGameDC:getDC():SendGameMsg(SUB_C_BAO_CARD, strMsg)
end
...
function InternetManager:sendBuckleCard(cardsData, cardsCount)
    local strMsg = "BYTE, 1," .. cardsCount .. ","
    for i = 0, cardsCount - 1 do
        strMsg = strMsg .. "BYTE, 1," .. cardsData[i] .. ","
    end
    print("InternetManager:sendBuckleCard:============> strMsg = " .. strMsg)
    FGameDC:getDC():SendGameMsg(SUB_C_BUCKLE_CARD, strMsg)
end

function InternetManager:sendJiaoCard(bJiao)
    -- body
    local strMsg = "bool, 1, " .. (bJiao and 1 or 0) .. ","
    FGameDC:getDC():SendGameMsg(SUB_C_JIAO_CARD, strMsg)
end
...
--发送规则设置
function InternetManager:sendScoreRule(inningNum, b1V3, bThreePlusTwo)
    -- body
    local str = string.format("BYTE,1,%d, BYTE,1,%d, BYTE,1,%d,", inningNum, b1V3, bThreePlusTwo)
    print(str)
    FGameDC:getDC():SendGameMsg(SUB_C_SCORE_RULE, str)
end

--发送解散消息
function InternetManager:sendDismissTable()
    -- body
    --local str = "BYTE, 1," .. chairId .. ","
    FGameDC:getDC():SendGameMsg(SUB_C_TABLEDISMISS, "")
end
...
--发送确认解散消息
function InternetManager:SendConfirmDismiss(bConfirm)
    -- body
    local nConfirm = bConfirm == true and 1 or 0
    FGameDC:getDC():SendGameMsg(SUB_C_RESPONSES_LEAVE, "BYTE,1," .. nConfirm .. ",")
end
